...
phenos <- list.files(path=dir.to.search,pattern=".22.categorical.qassoc")
toexclude <- list.files(path=dir.to.search,pattern="*adjusted")
phenos=setdiff(phenos,toexclude)
print(phenos) 
phenos <- gsub(".22.categorical.qassoc","",phenos)
top.snp.files <- NULL
...
for (pheno in phenos) {
    ...
    cmd <- paste0("cat ",dir.to.search, "/",pheno,"*.categorical.qassoc | awk '$9<5e-8{print $0}' | tr -s ' ' | sed 's/^ //' > ", top.snp.file)
    print(pheno)
    print(cmd) 
    system(cmd)
}
